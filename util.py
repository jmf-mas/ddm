...
def plot_uncertainty_bands(scores):
    ...
    def add_uncertainty(ax):
        ax.plot(scores_mean, distr_exp, '-', linewidth=2, color="#408765", label="predictive error mean")
        ax.fill_between(scores_mean, scores_mean - 2 * scores_std, distr_exp + 2 * scores_std, alpha=0.6, color='#86cfac', zorder=5)
